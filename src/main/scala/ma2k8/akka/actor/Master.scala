...
class Master extends Actor with ActorLogging {
  import ma2k8.akka.actor.PullArchitectureProtocol._

  val workers = List.empty[Worker]
//
...
//      if (!workers.contains(worker))
//        log.error("Oops! {} said it's done work but we didn't know about him", worker)
//      else
//        workers += (worker -> None)
//
...
//        log.error("Oops! {} died while processing {}", worker, workers(worker))
